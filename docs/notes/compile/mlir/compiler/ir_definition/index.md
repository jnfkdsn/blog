...
五章已完成阅读。当前反馈是单项机制能读懂，但跨文件、生成代码与注册过程难以连成统一模型。因此学习主线转到 [从 my.add 开始开发 Dialect](../../tutorials/my_dialect/)：每一步保留完整可运行版本，只增加一项能力，再做小修改。本目录继续作为机制解释与深入查阅入口；作者编译通过不等于学习者已独立实现。
...
精确范围继续由[覆盖表 M02—M03、X03](../../coverage)维护。通过增量工程接通定义与普通 Pattern 后，再进入 **Dialect Conversion**：以“自定义类型变成目标类型”为具体问题，处理合法性、操作替换及函数/区域边界。
