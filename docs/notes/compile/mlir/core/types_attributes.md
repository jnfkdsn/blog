...
先沿加法的输入追踪一次。`%x` 的内容来自调用者；另一个输入 `%seven` 则由前面的 constant 定义。加法接收的是这两个 Value，不能直接把文本数字 `7` 当作一个 operand 对象。

那么 constant 又从哪里得到 7？它没有输入操作数，而是在自身保存了整数 7 这份静态数据；操作语义规定，它的结果就是这个常量值。由此形成一条关系：**静态整数内容 → constant 操作 → 可被加法引用的 SSA 结果**。编译器读取静态内容，可以知道结果恒为 7；use-list 连接的仍是结果 Value 与使用者。

`i32` 则限制这条关系中的表示：这里保存的是一个有类型的整数属性，constant 产生 i32 结果，加法也在 i32 上运算。有了这条过程，再归纳各部分的职责：
