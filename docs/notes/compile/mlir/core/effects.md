...
回到第一节，假设编译器准备把第二次 load 替换成 `%before`。它需要证明两次读取看见同一个存储状态。两个 load 的地址相同，只完成了第一步；接下来还要检查中间有没有可能修改该位置的操作。这里的 store 写入同一个 `%buffer[%i]`，于是读取状态相同的前提不成立，必须保留第二次读取。

在这个小例子里，我们可以直接看出冲突。通用变换遇到陌生操作时，则需要查询“它会读什么、写什么”；若写入目标与读取位置不同，还可能结合别名分析判断是否重叠。若无法排除写入冲突，就不能据此复用旧读取。**效果信息提供决策需要的事实，变换算法再结合位置与依赖关系使用这些事实。**
